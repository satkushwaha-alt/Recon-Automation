...
import { getCellByAttributeAndValue, selectDebtByAttributeOnNv, selectEquitiesByAttributeOnNv, toggleOnForClosedBreaksOnNV, toggleOnForReconciledRecordsOnNV } from "../../lib/Locator/nvSetupLocator";
import { closeFilter, getCellByIdAndText, selectFilterByAttribute } from "../../lib/Locator/signoffLocator";
...
// await page.getByRole('textbox', { name: 'Email' }).fill('mayadav_recsignoff@ivp.in');
...
//   await page.getByRole('textbox', { name: 'Email' }).click();
//   await page.getByRole('textbox', { name: 'Email' }).fill('mayadav_9074@ivp.in');
//   await page.getByRole('textbox', { name: 'Instance' }).click();
//   await page.getByText('reconqaautouat').click();
...
await loginQA(page,"mayadav_recsignoff@ivp.in", "Ivp@123");
  ...
  await page.getByRole('textbox', { name: 'Search here' }).fill('Position_Nv_Setup_Automation');
  await page.getByRole('button', { name: 'Position_NV_Setup_Automation' }).click();
...
await page.locator('#Attributes').getByRole('textbox', { name: 'Quick Search' }).click();
await page.locator('#Attributes').getByRole('textbox', { name: 'Quick Search' }).fill('attribute-1');
await expect(page.locator(getCellByAttributeAndValue("attribute_name", "attribute-1"))).toContainText('attribute-1');
...
await expect(page.locator(getCellByAttributeAndValue("description", "edited"))).toContainText('edited');


// await page.locator(selectFilterByAttributeName("attribute_name")).click({force:true});
...
await page.getByRole('textbox', { name: 'Search...' }).fill('attribute-1edited');
...
await page.locator('#Attributes').getByRole('gridcell', { name: 'Press Space to toggle row' }).click();
await page.locator('#Attributes').getByText('Selected').click();
// await page.getByRole('checkbox', { name: 'Press Space to toggle row' }).check();
await expect(page.locator('#Attributes')).toContainText('Selected : 1');

await page.getByRole('gridcell', { name: 'Press Space to toggle row selection (checked)' }).click();
await page.locator(closeFilter("attribute_name")).click();
...
await page.locator('#Attributes').getByRole('textbox', { name: 'Quick Search' }).click();
  await page.locator('#Attributes').getByRole('textbox', { name: 'Quick Search' }).fill('Due Date');
...
// validate in mappings after deletion of attribute and edit of attribute
 await page.getByRole('row', { name: 'Position_Recon_NV_1 Position' }).getByLabel('Edit recon mappings').click();
  ...
  await expect(page1.locator('#wow_ReportsAndViews_container')).toContainText('attribute-1editedabc');
  await page1.getByRole('textbox', { name: 'Search Views' }).click();
  await page1.getByRole('textbox', { name: 'Search Views' }).fill('Due Date');
  await expect(page1.locator('#wow_ReportsAndViews_container')).toContainText('Account IDabcAmount_Spread123attribute-1editedabcContract Amount123CurrencyabcdateLIBOR Floor123PortfolioabcTest DateTrade Date');
  ...
  //validation on nv
  await expect(page1.getByRole('grid')).toContainText('attribute-1edited - Internal');
  await expect(page1.getByRole('grid')).toContainText('attribute-1edited - External');

   //reconcile check
    try {
      await page.getByRole("textbox", { name: "Quick Search" }).click();
      await page.getByRole("textbox", { name: "Quick Search" }).fill("same side system match");
      await expect(page.getByRole("grid")).toContainText("Same Side System Match");
      
      await page.getByRole("textbox", { name: "Quick Search" }).click();
      await page.getByRole("textbox", { name: "Quick Search" }).fill("same side manual match");
      await expect(page.getByRole("grid")).toContainText("Same Side Manual Match");
      await page.getByRole("textbox", { name: "Quick Search" }).click();
      await page
  ...
  //  await page1.getByTitle('Manage Recon Settings').click();
  // await page1.getByRole('button', { name: 'View Settings' }).click();
  // await page1.getByRole('button', { name: 'Recon Name' }).click();
  // await page1.getByRole('button', { name: 'Recon Status' }).click();
  // await page1.getByRole('button', { name: 'Break Type' }).click();
  // await page1.getByRole('button', { name: 'Break Status' }).click();
  // await page1.getByRole('button', { name: 'Approver', exact: true }).click();
  // await page1.getByRole('button', { name: 'Approver List' }).click();
  // await page1.getByRole('button', { name: 'Action Name' }).click();
  // await page1.getByTestId('generateViewSettings').click();
  //validate recon_name
  // await expect(page1.locator('body')).toContainText('External Exception (555)');
  // await page1.locator('[id="floatingFilter_Break Type_adaptableId"]').getByText('Select...').click();
  // await expect(page1.locator('body')).toContainText('Internal Exception (416)');
  // await page1.locator('[id="floatingFilter_Break Type_adaptableId"]').getByText('Select...').click();
  // await page1.getByText('Paired (11)').first().click();

// }catch(error){
//     console.error(`Error During Attribute Search, Edit, Delete validation`);
// }
});