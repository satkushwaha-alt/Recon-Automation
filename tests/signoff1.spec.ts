...
//validate
  await expect(page.getByLabel('BreakManagement -Copy of')).toContainText('BreakManagement -Copy of Break_Management_Position');
  await expect(page.locator('#ReconNameDiv')).toContainText('Copy of Break_Management_Position');
await expect(page.locator('#statusBarCount')).toContainText('9.6%');
  ...
  // await page.pause();
  await expect(page.locator('#signOffMatch')).toContainText('Match(%)By Record Count9.6%');
  await expect(page.locator('#signOffOpenBreaksText')).toContainText('Open Breaks');
  await expect(page.locator('#signOffClosedMatchText')).toContainText('Closed | Match (Manual)');
  await expect(page.locator('#signOffActionBreaksText')).toContainText('Uncommented Breaks | Unassigned Break Status Count');
  ...
  await page.locator('.ag-row-even.ag-row-no-focus > div:nth-child(4) > .ag-cell-wrapper').first().click();
  await page.locator('.ag-row-even.ag-row-no-focus > div:nth-child(4) > .ag-cell-wrapper').first().click({
    button: 'right'
  });
  await expect(page.getByRole('menu')).toContainText('MatchUpdate RemarksClose BreakWorkflow Actions Insert MappingMove to AssetAttachment CommunicateTrace RecordAudit TrailPostSearch');
  //validation after signoff
  await page.getByRole('button', { name: 'Sign off' }).click();
 
  // signoff background color validation
  const signoff = page.locator('#signOffApproveButton').getByRole('button', { name: 'Sign off' });
  const signoffbgColor = await signoff.evaluate(el => 
  ...
  // const rejectsignoff = page.locator('#rejectsignoff').getByRole('button', { name: 'Sign off' });
  // const rejectsignoffbgColor = await rejectsignoff.evaluate(el => 
  //   window.getComputedStyle(el).backgroundColor
  // );
  ...
  // const approvesignoff = page.locator('#approvesignoff').getByRole('button', { name: 'Sign off' });
  // const approvesignoffbgColor = await approvesignoff.evaluate(el => 
  //   window.getComputedStyle(el).backgroundColor
  // );
  ...
  // await page.locator('.ag-row-even.ag-row-no-focus.ag-row.ag-row-level-0.ag-row-position-absolute.ag-row-hover > div:nth-child(4) > .ag-cell-wrapper').click();
  // await page.mouse.move(100,100);
  await page.locator('.ag-row-even.ag-row-no-focus.ag-row.ag-row-level-0.ag-row-position-absolute.ag-row-hover > div:nth-child(4) > .ag-cell-wrapper').click({
    button: 'right'
  });
  await expect(page.getByLabel('Context Menu').getByRole('menu')).toContainText('Attachment');
  await expect(page.getByRole('menu')).toContainText('Trace Record');
  await expect(page.getByRole('menu')).toContainText('Audit Trail');
  await expect(page.getByLabel('Context Menu').getByRole('menu')).toContainText('Post');
  ...
  await expect(page.locator('#signOffOpenBreaksText')).toContainText('Open Breaks');
  await expect(page.locator('#signOffClosedMatchText')).toContainText('Closed | Match (Manual)');
  await expect(page.locator('#SignOffStatsDiv')).toContainText('Uncommented Breaks | Unassigned Break Status Count');
  await expect(page.locator('#signOffCancelButton')).toContainText('Cancel');
  ...
  await page.getByRole('button', { name: 'Copy of' }).click();
  await expect(page.locator('#RS_FirstRow')).toContainText('Sign off Status :');
  await expect(page.locator('#RS_FirstRow')).toContainText('Revoke Sign off Pending');
  await expect(page.locator('#RS_FirstRow')).toContainText('Reject');
  await expect(page.locator('#RS_FirstRow')).toContainText('Approve');
  await page.getByTestId('runSatusClose').click();
  //Reject Revoke
  ...
  await page.getByRole('button', { name: 'Copy of' }).click();
  await expect(page.locator('#RS_FirstRow')).toContainText('Sign off Status :');
  await expect(page.locator('#RS_FirstRow')).toContainText('Sign Off Not Intiated');
  await expect(page.locator('#RS_FirstRow')).toContainText('Sign Off');
  await page.getByTestId('runSatusClose').click();
