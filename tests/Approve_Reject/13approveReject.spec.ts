...
import { facilityId100ExternalException } from "../../Static_Files/ApproveReject/data";
import { selectRowByValues } from "../../lib/Approvereject/selectrowByValues";
import { agregateAndSameSideMatchTogle } from "../../lib/Locator/approveRejectLocator";
//External Exception - Aggregate 100 records
test("approveReject13", async ({ page }) => {
  ...
  await selectRowByValues(page, facilityId100ExternalException);
  //aggregate action
  await (page
    .getByText(
      facilityId100ExternalException[facilityId100ExternalException.length - 1],
    )).first()
    .click({
      button: "right",
    });
  ...
  await page
    ...
    .fill("aggregate and match by satyam");
  //toggle On - Aggregate and Same Side Match
  await page.locator(agregateAndSameSideMatchTogle()).click();
  ...
  try {
    await page.locator(selectFilterByAttribute("Status")).click();
    await expect(page.locator("body")).toContainText("Open (6045)");
    await expect(page.locator("body")).toContainText("Pending (1)");
    await page.getByLabel("", { exact: true }).nth(2).click();
    await page.getByText("aggregate and match by satyam").click();
  ...
  await page.locator(selectFilterByAttribute("Status")).click();
  try {
    await page.getByText("Open (6046)").click();
    await expect(page.locator("#ReconGrid")).toContainText(
      "Total Rows : 6,424",
    );
  } catch (e) {
