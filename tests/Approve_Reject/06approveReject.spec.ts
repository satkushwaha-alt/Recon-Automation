...
import { facilityId50ExternalException } from "../../Static_Files/ApproveReject/data";
import { selectRowByValues } from "../../lib/Approvereject/selectrowByValues";
//External Exception - Aggregate 50 records
test("approveReject6", async ({ page }) => {
  ...
  await selectRowByValues(page, facilityId50ExternalException);
  //aggregate action
  await page
    .getByText(
      facilityId50ExternalException[facilityId50ExternalException.length - 1],
    )
    .click({
      button: "right",
    });
