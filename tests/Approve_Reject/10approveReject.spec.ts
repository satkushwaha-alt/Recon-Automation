...
import { facilityId100InternalException } from "../../Static_Files/ApproveReject/data";
import { selectRowByValues } from "../../lib/ApproveReject/selectRowByValues";
//Internal Exception - Aggregate 100 records
test("approveReject10", async ({ page }) => {
  ...
  await selectRowByValues(page, facilityId100InternalException);
  //aggregate action
  await page
    .getByText(
      facilityId100InternalException[facilityId100InternalException.length - 1],
    )
    .click({
      button: "right",
    });
