...
import {
  dropdownButtonNvOnReconMapping,
  selectDebtByAttributeOnNv,
  selectEquitiesByAttributeOnNv,
} from "../Locator/nvSetupLocator";
export const updateReconMappingInNV = async (
  page: any,
  nv_name: string,
  recon_name: string,
) => {
  try {
    ...
    await page.evaluate(
      (el: { scrollIntoView: (arg0: { block: string }) => void }) => {
    ...
    for (let i = 0; i < mappings.length; i++) {
      ...
      await page
        .locator(selectDebtByAttributeOnNv(`${mappings[i].attribute_name}`))
        .click();
      ...
      //for Equities
      await page
        .locator(selectEquitiesByAttributeOnNv(`${mappings[i].attribute_name}`))
        .click();
      await page.getByRole("combobox").fill(`${mappings[i].equities}`);
      await page
        .getByRole("option", { name: `${mappings[i].equities}` })
        .click();
    }
  ...
  page.waitForResponse((resp: { url: () => string | string[]; }) =>
    resp.url().includes('/RSetup/api/SaveChangesInCache')
  ),
   await page.getByRole("button", { name: "Save", exact: true }).click(),
...
if (status !== 200) {
  throw new Error(`❌ API failed: ${status}`);
    ...
    console.log(
      `Recon mapping updated in NV successfully for ${nv_name} and ${recon_name} and status = ${status}`,
    );
    await page.waitForTimeout(3000);
  } catch (error) {
